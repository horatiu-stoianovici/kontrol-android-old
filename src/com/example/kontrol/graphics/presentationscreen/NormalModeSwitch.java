...
public class NormalModeSwitch extends HBaseGraphics {
	...
	public NormalModeSwitch(Context context, TouchpadView view){
		buttonDrawable = context.getResources().getDrawable(R.drawable.mouse_icon);
		pressedDrawable = context.getResources().getDrawable(R.drawable.mouse_icon);
		delegate = view;
	...
	public boolean onMotionEvent(MotionEvent event, float offset) {
		...
		case MotionEvent.ACTION_POINTER_UP:
			if(activePointerId == pressedId){
				pressedId = -1;
				delegate.switchToContext(HContext.TouchpadScreen);
				return true;
			}
			break;
		}
